...
open class Router<InteractorType>: Routing {
  ...
  /// The base `Interactable` associated with this `Router`.
  public let interactable: any Interactable
  ...
  public init(interactor: InteractorType) {
    ...
    guard let interactable = interactor as? (any Interactable) else {
      fatalError("\(interactor) should conform to \((any Interactable).self)")
    }
